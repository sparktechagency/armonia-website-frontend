...
import { Review } from "@/redux/features/auth/authSlice";
...
export default function Testimonials({ data = [] }: { data: Review[] }) {
  ...
  return (
    <section className="px-3 2xl:px-36 py-16 text-center bg-yellow-50">
      ...
      <Swiper
        ...
        onInit={(swiper) => {
          ...
          // @ts-ignore (Swiper's API doesn't have perfect TS support for refs)
          ...
          <SwiperSlide key={item.id}>
            <div className="my-14 lg:min-h-[688px] bg-blue-500 rounded-[40px] flex flex-col lg:flex-row items-center justify-around gap-10 md:gap-20 p-10">
              <div className="flex justify-center max-w-xl w-full">
                ...
                <div className="w-[200px] lg:w-[280px] h-[200px] md:h-[280px] my-auto rounded-full border-4 border-white overflow-hidden flex items-center justify-center">
                  <Image
                    src={
                      item.user?.image
                        ? `${process.env.NEXT_PUBLIC_API_URL}${item?.user?.image}`
                        : "/profile-demo.png"
                    }
                    alt="quote"
                    width={280}
              ...
              <div className="text-white max-w-xl text-start relative my-6 py-2">
                ...
                <p className="lg:text-xl mt-5">&ldquo;{item.review}&ldquo;</p>
                <svg
                  width="46"
                  height="41"
                  viewBox="0 0 46 41"
      ...
      <div className="flex gap-4 items-center justify-center">
        {/* Left Button */}
        <button   ref={prevRef} className="flex items-center justify-center w-12 h-12 lg:w-16 lg:h-16 rounded-full bg-white shadow hover:bg-gray-200 transition">
